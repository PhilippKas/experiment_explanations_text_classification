...
def test_al(labeld_x, labeld_y, unlabeld_x, unlabeld_y, test_x, test_y, dps_to_label):
    ...
    while len(labeld_x) < 1500:
        ...
        svc = SVC(kernel="linear", class_weight=weights, probability=True)
...
test_al(
    np.empty((0,count_vect_train.shape[1]), dtype = count_vect_train.dtype),
    np.empty((0,labels_onehot_train.shape[1]), dtype = labels_onehot_train.dtype),
    count_vect_train,
    labels_onehot_train,
    count_vect_test, 
    labels_onehot_test,
    np.random.choice(len(count_vect_train), 25, replace=False))
...
with open("results/uncert_svm_bow_2.json", "w") as outfile:
