...
for al_samplesize in n_to_test:
    ...
    kmeans.fit(count_vect_train)
    closest, _ = pairwise_distances_argmin_min(kmeans.cluster_centers_, count_vect_train)
    
    tfidf_vect_train_selected = count_vect_train[closest]
    labels_onehot_train_selected = labels_onehot_train[closest]

    weights = {0:1.0, 1:100.0}
    kmeans_svc = SVC(kernel="linear", class_weight=weights, probability=True)
    ...
    multilabel_classifier_kmeans_svc.fit(
        tfidf_vect_train_selected,
        labels_onehot_train_selected_valid)

    # Compute predictions on the test data for the valid labels 
    labels_onehot_test_predicted_valid = multilabel_classifier_kmeans_svc.predict(count_vect_test)
...
with open("results/kmsubset_svm_bow.json", "w") as outfile:
